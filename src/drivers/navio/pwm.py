...
Modified by Dilanka Dharmasena
'''
...
class PWM():
    ...
    def initialize(self):
        ...
        if not os.path.exists(self.channel_path):
            ...
            if self.channel == 0:  # not sure if it begins on a newline or not
                with open(self.SYSFS_PWM_EXPORT_PATH, "a") as pwm_export:
                    pwm_export.write(str(self.channel))
                with open(self.SYSFS_PWM_EXPORT_PATH, "a") as pwm_export:
                    pwm_export.write(str(1))
                with open(self.SYSFS_PWM_EXPORT_PATH, "a") as pwm_export:
                    pwm_export.write(str(2))
                with open(self.SYSFS_PWM_EXPORT_PATH, "a") as pwm_export:
                    pwm_export.write(str(3))
